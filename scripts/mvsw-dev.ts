...
export async function writeDevManifest(): Promise<void> {
  try {
    // copy icon files
    ...
    await fs.move(resolve('extension/dev/dist/content'), resolve('extension/dev/content'), {
      overwrite: true,
    });
